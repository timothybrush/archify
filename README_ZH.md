...
Archify 基于 [Cocoon-AI/architecture-diagram-generator](https://github.com/Cocoon-AI/architecture-diagram-generator) v1.0（只有深色主题的 HTML 输出）fork 重写。**2.0** 把模板重构成 CSS 变量驱动的可主题化系统，加入客户端导出流水线。**2.1** 加入剪贴板复制 + 键盘导航。**2.2** 加入打印样式 + 本地字体回退。**2.3** 修了位图升采样 bug，所有光栅导出改为最高 4× 原生渲染。**2.4** SVG 导出升级成双主题自持版。**2.5** 加入 workflow / sequence / data-flow / lifecycle 四种渲染器、Mermaid 输入指引、CJK-aware 文本测量、golden 测试和 CI。**2.6** 把 architecture 模式也升级到 schema + layout 校验。**2.7** 强化 workflow：phase header、group、exception lane、mainPath 主路径 lint、同泳道正交连线，以及生成后 HTML/SVG 检查器，交付前自动抓出斜线和图例穿插问题。

| 能力 | v1.0 | 2.0 | 2.1 | 2.2 | 2.3 | 2.4 | 2.5 | 2.6 | 2.7 |
|---|---|---|---|---|---|---|---|---|---|
| 深色主题 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| 浅色主题 | — | 切换 | 切换 | 切换 | 切换 + <kbd>T</kbd> 快捷键 | 同 | 同 | 同 | 同 |
| PNG / JPEG / WebP 下载 | 手动截图 | 2× 位图放大 | 1×/2×/4× 选择器（仍是放大）| 同 | **4× 原生光栅化，不糊** | 同 | **浅色导出泳道配色修复** | 同 | 同 |
| SVG 下载 | — | 矢量 + 内联样式（单主题）| 同 | 同 | 同 | **双主题自持**（`@media prefers-color-scheme`）| 同（泳道配色修复）| 同 | 同 |
| 复制 PNG 到剪贴板 | — | — | ✓ | 同 | 同（最高 4×）| 同 | **Safari 修复** | 同 | 同 |
| 键盘快捷键 | — | — | <kbd>T</kbd>/<kbd>E</kbd> + 菜单导航 | 同 | 同 | 同 | 同 | 同 | 同 |
| 可访问性 | — | — | ARIA + focus-visible | 同 | 同 | 同 | 同（+ 菜单 a11y 修复）| 同 | 同 |
| 打印样式表 | — | — | — | ✓ | ✓（+ 横向 + 2 列卡片）| 同 | 同 | 同 | 同 |
| 导出时本地字体回退 | — | — | — | ✓ | ✓ | 同 | **+ CJK 字体回退** | 同 | 同 |
| 样式模型 | 内联 `fill` / `stroke` | CSS 变量 + 语义 class | 同 | 同 | 同 | 同 | 同 | 同 | 同 |
| 类型化渲染器 + schema 校验 | — | — | — | — | — | — | workflow / sequence / data-flow / lifecycle | **+ architecture** | 同 |
| Workflow 结构辅助 | — | — | — | — | — | — | 泳道 + 路由连线 | 同 | **phase、group、exception lane、mainPath lint** |
| 生成后 artifact 检查 | — | — | — | — | — | — | — | — | **✓** |
...
- **生成后检查**：`scripts/check-render-output.mjs` 在交付前检查生成的 HTML：单 SVG、SVG 坐标有限、没有误生成两点斜线箭头、箭头线段不穿过图例。
